...
const tsoptions = getJSON(path.resolve(process.cwd(), 'tsconfig.json'))
...
const tasks = {
  ...
  , dist: function* (task) {
    if (!fs.existsSync('Manifest.json')) {
      // A simple module, We just need to compile de code to lib
      yield task.serial(['build'])
      return
    }
  ...
  , register: function* (task) {
      yield task.source(path.resolve(distPath,'bundle.zip'))
